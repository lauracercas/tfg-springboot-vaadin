...
window.Vaadin.Flow['_vaadintheme_tfgspringvaadin_globalCss'] = window.Vaadin.Flow['_vaadintheme_tfgspringvaadin_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_tfgspringvaadin_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_tfgspringvaadin_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_tfgspringvaadin_globalCss'].push(target);
  }
  if (!document['_vaadintheme_tfgspringvaadin_componentCss']) {
    
    document['_vaadintheme_tfgspringvaadin_componentCss'] = true;
  }
